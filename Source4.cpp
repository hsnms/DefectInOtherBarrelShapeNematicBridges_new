...
int main()
{
    int i,j,k,n,ncycle=5,ii,ii2,ii3,ii4,sos,kk,radius=1,min,n1,RR;
...
for (j=1;j<=(int)(n*H-H+1);j++)
		{
		  ...
		  qq[j]=round(n*q[j]-q[j]+1); printf("qq[%d]=%d\n",j,qq[j]);
 ...
 for(j=1;j<=(int)(n*H-H+1);j++)
  {
	...
	for (i=1+b;i<=qq[1];i++)
               ...
               for (j=2;j<=(int)(n*H-H+1)-1;j=j+1)
		{
			     ...
			     u[qq[j]][j]=Right-atan(2.0/((double)((-qq[j+1]+qq[j-1]))));
			 u[1][j]=Left-0.5*m*Pi;
			}
	       for (j=(int)(n*H-H+1)-1; j>=2;j--)
		 {
		   for (i=2;i<=qq[j]-1;i++)
		     {
		       if (i>=qq[j+1])/*barrel*/
			 u[i][j]=u[qq[j+1]][j+1]+(u[qq[j]][j]-u[qq[j+1]][j+1])*(i-qq[j+1]+1)/(qq[j]-qq[j+1]+1);/*barrel*/
		     }
		 }
